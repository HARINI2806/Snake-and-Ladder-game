...
int main() //this is where it all goes down in c++, it's like the canvas for da vinci...
{
    ...
    do // every game is in some sort of loop
    {
        ...
        roll = s.roll_dice();//player rolls dice
        ...
        computer_roll = s.roll_dice();//computer rolls dice
        ...
        squares[player] = 111; //player's position shall be represented by the integer 111
        squares[computer] = 222; //computer's position shall be represented by the inteer 222
...
void snake:: board()//define the board drawing mechanism!
{
    cout << "----------------------------------------------------------------------------"<<endl<<endl;
...
int snake:: roll_dice()//define the dice rolling mechanism
{
    return rand() % (7-1) + 1;//can only roll 1 - 6
}

int snake:: check_progress()//define the progress checking mechanism
{
    if ( squares[100] == 111 || squares[100] == 222)
    {
